...
int main() {
	int n, a[N], b[N], m, s, t, total_gain = 0;
	dinic_init();
	scanf("%d", &n);
	for (int i = 0; i < n; i++) { scanf("%d", &a[i]); total_gain += a[i]; }
	for (int i = 0; i < n; i++) { scanf("%d", &b[i]); total_gain += b[i]; }
	scanf("%d", &m);
	s = 0;
	t = 1 + n + 2 * m;
	for (int i = 0; i < n; i++){
		add_edge(s, i + 1, a[i], 0);
		add_edge(i + 1, t, b[i], 0);
	}
	for (int i = 0; i < m; i++){
		int k;
		scanf("%d", &k);
		...
		total_gain += c1;
		total_gain += c2;
		add_edge(s, 1 + n + i, c1, 0);
		add_edge(1 + n + m + i, t, c2, 0);
		for (int p = 0; p < k; p++){
			int ttt;
			scanf("%d", &ttt);
			add_edge(1 + n + i, ttt, INF, 0);
			add_edge(ttt, 1 + n + m + i, INF, 0);
		}
	}
	int flow = dinic(s, t);
	printf("%d\n", total_gain - flow);
